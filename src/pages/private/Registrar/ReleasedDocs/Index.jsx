...
function ReleasedDocs() {
        ...
        const getUsers = async () => {
            try {
                ...
                const { data } = await http.get(
                    "/api/blockchainUploadFileReleasedDocs"
                );
                setSearchText("");
                setReleasedDocs(data);
                setFilteredReleasedDocs(data);
            } catch (error) {
                setError(error);
            } finally {
    ...
    if (userStatus === "For Verification") {
        return (
            <>
                <h1 className="is-size-4 mb-4">Released Docs</h1>
    ...
    const handleSearch = () => {
        if (searchText.trim() === "") {
            setFilteredReleasedDocs(releasedDocs);
        } else {
            const filteredReleasedDocs = releasedDocs.filter(releasedDoc => {
                const {
                    student_lastname,
                    student_firstname,
                    student_middlename,
            ...
            setFilteredReleasedDocs(filteredReleasedDocs);
        }
    };
        ...
        <>
            ...
            <div className="box" style={{ position: "relative" }}>
                <div className="is-flex is-justify-content-space-between">
                    <div></div>
                    <div>
                        <Link to="/uploadReleasedDoc">
                            <button className="button is-success">
                                Upload released docs
                            </button>
                        </Link>
                    </div>
                </div>
                ...
                <div className="is-flex">
                    <input
                        ...
                        placeholder="Search student name or file name"
                        value={searchText}
                ...
                <hr />
                {filteredReleasedDocs.length === 0 ? (
                    <div className="has-text-centered p-4">
                        No released docs found.
                    </div>
                ) : (
                    <>
                        ...
                        <div className="p-4 has-text-right">
                            {filteredReleasedDocs.length} total items
                        </div>
                        {isReadLoading && (
                            <div
                                style={{
